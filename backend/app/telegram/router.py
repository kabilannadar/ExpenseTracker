...
from app.telegram.parser import parse_expense_message, format_help_message
...
async def telegram_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    # 5. Save based on statement type
    if parsed.statement_type == "income":
        ...
        db.add(AuditLog(
            ...
            entity_type="income",
            entity_id=income.id,
            detail=f"Added via Telegram: '{parsed.title}' ₹{income.amount}"
        ))
        db.commit()
        ...
        return {"status": "success", "income_id": income.id}
        ...
        db.add(AuditLog(
            ...
            entity_type="emi",
            entity_id=emi.id,
            detail=f"Added via Telegram: EMI '{emi.title}' ₹{emi.emi_amount}"
        ))
        db.commit()
        ...
        return {"status": "success", "emi_id": emi.id}
        ...
        db.add(AuditLog(
            ...
            entity_type="debt",
            entity_id=debt.id,
            detail=f"Added via Telegram: Debt '{debt.creditor}' ₹{debt.amount}"
        ))
        db.commit()
        ...
        return {"status": "success", "debt_id": debt.id}
        ...
        db.add(AuditLog(
            ...
            entity_type="goal",
            entity_id=goal.id,
            detail=f"Created via Telegram: Goal '{goal.title}' Target ₹{goal.target_amount}"
        ))
        db.commit()
        ...
        db.add(AuditLog(
            ...
            entity_id=sub.id,
            detail=f"Created via Telegram: Subscription '{sub.name}' ₹{sub.amount}"
        ))
        db.commit()
        ...
        db.add(AuditLog(
            ...
            detail=f"Created via Telegram: Reminder '{reminder.title}'"
        ))
        db.commit()
        ...
        if existing_cat:
            send_telegram_reply(chat_id, f"⚠️ Category *{existing_cat.name}* already exists!")
        ...
        db.add(AuditLog(
            ...
            entity_id=cat.id,
            detail=f"Created via Telegram: Category '{cat.name}'"
        ))
        db.commit()
        ...
        db.add(AuditLog(
            ...
            detail=f"Set via Telegram: Budget limit ₹{parsed.amount} for {cat_label}"
        ))
        db.commit()
        ...
        return {"status": "success", "budget_id": budget_obj.id}
        ...
        db.add(AuditLog(
            ...
            detail=f"Created via Telegram: Recurring '{recurring.title}' ₹{recurring.amount} ({freq_enum.value})"
        ))
        db.commit()
        ...
        return {"status": "success", "recurring_id": recurring.id}
        ...
        db.add(AuditLog(
            ...
            detail=f"Created via Telegram: {expense.title} - ₹{expense.amount}"
        ))
        db.commit()
